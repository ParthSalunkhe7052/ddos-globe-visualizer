...
from dshield_service import fetch_dshield_top_ips, fetch_dshield_events, normalize_dshield_event, generate_mock_dshield_events
from geo_service import ip_to_location
from ip_cache import get_cached, set_cache
from error_handler import (
    setup_error_handlers, APIError, InvalidIPError, RateLimitError,
...
LIVE_CACHE: Dict[str, Any] = {"lock": threading.Lock(), "data": [], "timestamp": time.time()}
...
async def enrich_ip(ip: str, use_abuseipdb: bool = False) -> dict:
    ...
    if (
        ...
        and ABUSEIPDB_KEY
        and (
            not AbuseIPDB429["blocked_until"] or now > AbuseIPDB429["blocked_until"]
        )
    ):
        try:
            async with httpx.AsyncClient() as client:
...
async def set_feed_mode(mode: str = Query(..., pattern="^(live|fallback)$")):
    ...
    if mode not in ("live", "fallback"):
        return log_and_respond(False, error="INVALID_MODE", message="mode must be 'live' or 'fallback'", status_code=400)
    FEED_MODE = mode
...
async def enrich_ip_endpoint(ip: str, abuse: bool = False):
    ...
    try:
        ...
        raise ServiceUnavailableError("AbuseIPDB", {"status_code": e.response.status_code})
    except Exception as e:
...
async def cluster_expand(
    ...
    try:
        ...
        if cluster_id:
            group = [
                a for a in attacks if str(a.get("cluster_id")) == str(cluster_id)
            ]
            return {
                "success": True,
                "data": {
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                await handle_ws_error(websocket, APIError(
...
async def debug_feed_mode(mode: str = Query("live", description="Feed mode: live, fallback, or mock")):
