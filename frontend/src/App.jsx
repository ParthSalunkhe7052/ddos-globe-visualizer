...
const GLOBE_TEXTURES = [
  ...
  { label: 'Dark', url: 'https://unpkg.com/three-globe/example/img/earth-dark.jpg' }
];

// Fallback texture in case remote textures fail to load
const FALLBACK_TEXTURE = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+P+/HgAFhAJ/wlseKgAAAABJRU5ErkJggg==';
...
function SideInfoPanel({ info, collapsed, onToggle, isMobile }) {
  ...
  const panelStyle = {
    ...
    background: 'linear-gradient(180deg, rgba(16,16,16,0.98), rgba(12,12,12,0.96))',
  ...
  const renderField = (label, value) => value !== undefined && value !== null && value !== '' && (
    <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: 6, alignItems: 'center' }}>
      <div style={{ color: '#FFD700', opacity: 0.9, fontWeight: 600 }}>{label}</div>
    ...
    <div style={panelStyle} className="side-info-panel">
      ...
      {!collapsed && (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          {/* Sticky header */}
          <div style={{ position: 'sticky', top: 0, background: 'rgba(18,18,18,0.98)', borderBottom: '1px solid rgba(255,213,79,0.25)', padding: '14px 16px', zIndex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 10, height: 10, borderRadius: 10, background: '#FFD54F', boxShadow: '0 0 8px rgba(255,213,79,0.5)' }} />
              <div style={{ fontWeight: 700, letterSpacing: 0.4 }}>IP Details</div>
            </div>
          </div>

          {/* Scrollable content */}
          <div className="side-info-content" style={{ padding: '12px 16px', fontSize: 13.5, lineHeight: 1.6, display: 'grid', gap: 12, overflowY: 'auto' }}>
            {info ? (
              <>
                ...
                {renderField('Last Seen', info.lastSeen || abuse.lastReportedAt)}
                {renderField('Confidence', (info.confidence !== undefined ? info.confidence : abuse.abuseConfidenceScore) + '%')}
                {typeof info.lat === 'number' && typeof info.lng === 'number' && (
                  renderField('Coordinates', `${info.lat.toFixed(3)}, ${info.lng.toFixed(3)}`)
                )}
                {renderField('Hostnames', Array.isArray(info.hostnames) ? info.hostnames.join(', ') : info.hostnames)}
              </>
            ) : (
          ...
          <div style={{ position: 'sticky', bottom: 0, background: 'rgba(18,18,18,0.98)', borderTop: '1px solid rgba(255,213,79,0.25)', padding: '10px 12px', display: 'flex', gap: 8 }}>
            {info?.ip ? (
              <>
                <a href={`https://www.abuseipdb.com/check/${info.ip}`} target="_blank" rel="noreferrer" className="ip-button" style={{ textDecoration: 'none' }}>AbuseIPDB</a>
                <a href={`https://ipinfo.io/${info.ip}`} target="_blank" rel="noreferrer" className="ip-button" style={{ textDecoration: 'none' }}>IPInfo</a>
              </>
            ) : (
              <button type="button" className="ip-button" disabled style={{ opacity: 0.5 }}>No IP</button>
            )}
          </div>
        </div>
      )}
...
function AppContent() {
  ...
  const addDShieldArc = useCallback((arc) => {
    ...
    setRings(prev => {
      const newRing = { id: `ring-${arc.id}`, lat: arc.endLat, lng: arc.endLng };
  ...
  const handleDShieldStatus = useCallback((status) => {
    if (status.error) {
      showToast && showToast(`DShield stream error: ${status.error}`, 'error');
  ...
  const { isConnected: dshieldConnected, lastError: dshieldError } = useDShieldStream(
    liveMode,
    addDShieldArc,
    handleDShieldStatus
  );
  ...
  useEffect(() => {
    ...
    const allowRandomArcs = (import.meta?.env?.MODE === 'development') &&
    ...
    if (allowRandomArcs) {
      ...
      const generateRandomArc = () => {
        ...
        const color = severity > 0.7 ? 'red' : severity > 0.4 ? 'orange' : 'yellow';
      ...
      const interval = setInterval(generateRandomArc, 2000 + Math.random() * 3000);
  ...
  useEffect(() => {
    ...
    const loadTexture = async () => {
      try {
        setLoading(true);
        const textureUrl = await loadTextureWithRetry([globeTexture, ...GLOBE_TEXTURES.map(t => t.url)]);
        if (mounted) {
  ...
  async function handleAnalyze(targetIp) {
    ...
    fetch(`http://127.0.0.1:8000/analyze_ip?ip=${encodeURIComponent(targetIp)}`)
      ...
      .then(data => {
        ...
        window.history.replaceState({}, '', window.location.pathname + (targetIp ? `?ip=${targetIp}` : ''));
  ...
  return (
    <NotificationProvider>
      <div className="app-container" style={{ width: '100vw', height: '100vh', background: '#111', position: 'relative', overflow: 'hidden' }}>
        ...
        <div role="application" aria-label="Interactive globe" className="globe-container">
          <ErrorBoundary>
