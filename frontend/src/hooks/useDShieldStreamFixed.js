...
export default function useDShieldStreamFixed(liveMode, addArc, onStatus) {
    ...
    const connect = useCallback(() => {
        ...
        try {
            ...
            ws.onopen = () => {
                console.log('[useDShieldStreamFixed] ✅ Connected to DShield stream at:', wsUrl);
                setIsConnected(true);
                setLastError(null);
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (msg && msg.type === 'attack' && msg.data) {
                        ...
                        console.log('[useDShieldStreamFixed] 🎯 Attack event received:', event.id);
                    ...
                    console.error('[useDShieldStreamFixed] Failed to parse message:', e, event.data);
                }
            };
            ...
            ws.onclose = (event) => {
                ...
                if (liveMode && reconnectAttempts.current < maxReconnectAttempts) {
                    ...
                    console.log(`[useDShieldStreamFixed] Reconnecting in ${backoff}ms (attempt ${reconnectAttempts.current})`);

                    reconnectTimeoutRef.current = setTimeout(() => {
                        connect();
                    }, backoff);
                } else if (reconnectAttempts.current >= maxReconnectAttempts) {
                    console.error('[useDShieldStreamFixed] Max reconnection attempts reached');
            ...
            console.error('[useDShieldStreamFixed] Failed to create WebSocket:', error);
    ...
    const processPendingEvents = useCallback(() => {
        ...
        if (timeSinceLastArc >= arcInterval && pendingEventsRef.current.length > 0) {
